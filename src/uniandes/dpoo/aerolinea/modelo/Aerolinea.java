...
public class Aerolinea
{
    ...
    {
		Iterator<Vuelo> iter = vuelos.iterator();
		while (iter.hasNext()) {
    ...
    public void cargarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException
    {
		...
		try {
			cargador.cargarAerolinea(archivo, this);
				...
				throw new InformacionInconsistenteException("Aerolinea");
	...
	public void cargarTiquetes( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException
	{
	    ...
	    try {
			...
			throw new InformacionInconsistenteException("Tiquetes");
	    }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        if (getVuelo(codigoRuta, fecha) != null) {
        	throw new Exception("Ya existe un vuelo con la misma fecha y ruta");
			...
			if (avion == null) {
				throw new Exception("No se encontró el avión");
			}
    ...
    {
    	List<String> temporadaBaja = new ArrayList<String>();
    	Collections.addAll(temporadaBaja, "enero", "febrero", "marzo", "abril", "mayo");
    	List<String> temporadaAlta = new ArrayList<String>();
    	Collections.addAll(temporadaAlta, "septiembre", "octubre", "noviembre");
    	Vuelo vuelo= getVuelo(codigoRuta, fecha);
		Cliente cliente = getCliente(identificadorCliente);
		if (cantidad < getVuelo(codigoRuta, fecha).getAvion().getCapacidad()) {
			if (cliente != null && vuelo != null) {
				CalculadoraTarifasTemporadaBaja calculadora1 = new CalculadoraTarifasTemporadaBaja();
				CalculadoraTarifasTemporadaAlta calculadora2 = new CalculadoraTarifasTemporadaAlta();
				
				for (int i = 0; i < cantidad; i++) {
					int tarifa1 = 0;
					if (temporadaBaja.contains(fecha.split(" ")[1]) ) {
						tarifa1 = calculadora1.calcularTarifa(vuelo, cliente);
					} else {
						tarifa1 = calculadora2.calcularTarifa(vuelo, cliente);
					}
					Tiquete tiquete = GeneradorTiquetes.generarTiquete(vuelo, cliente, tarifa1);
    ...
    public void registrarVueloRealizado(String fecha, String codigoRuta) {
        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo == null) {
            System.out.println("No se encontró el vuelo con la fecha y código de ruta proporcionados");
        } else if (!vuelos.contains(vuelo)) {
            vuelos.add(vuelo);
        } else {
            System.out.println("El vuelo ya fue registrado");
        }
    }
